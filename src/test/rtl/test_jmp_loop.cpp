...
int main() {
    ...
    FILE* f = fopen("basic/test_jmp_loop.bin", "rb");
    if (!f) { printf("Can't open test_jmp_loop.bin\n"); return 1; }
    ...
    printf("Test: JMP loop\n");
    printf("Expected: Output 'H' repeatedly\n\n");
    ...
    for (int cycle = 0; cycle < 500 && output_count < 5; cycle++) {
        uint16_t addr = cpu->addr;
        uint16_t pc = cpu->pc_out;
        
        if (pc != last_pc && pc >= 0x0300 && pc < 0x0310) {
            printf("[PC=%04X] %02X\n", pc, mem[pc]);
            last_pc = pc;
        }
    ...
    printf("\nTest complete - outputted %d characters\n", output_count);
    delete cpu;
    return 0;
}
